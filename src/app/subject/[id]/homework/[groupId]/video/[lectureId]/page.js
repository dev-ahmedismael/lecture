...
export default function HomeworkVideoPage() {
    ...
    async function fetchLecture() {
      ...
      try {
        const res = await fetch(
          `https://eng-mohamedkhalf.shop/api/HomeWorks/GetHomeWorkLectureDetails/${lectureId}`,
          {
            headers: {
  ...
  const handleSendComment = async () => {
    ...
    try {
      const res = await fetch(
        "https://eng-mohamedkhalf.shop/api/HomeWorks/AddComment",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
